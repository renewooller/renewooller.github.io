...
public class MidiLocationSetter extends JPanel implements Serializable {
	...
	/*
	 * private NumberTextField chan = new NumberTextField(1, 16, 1); private
	 * NumberTextField ctrl = new NumberTextField(0, 127, 0);
	 */
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == chan || e.getSource() == ctrl) {
					if (midiLocation instanceof MidiInputLocation) {
						mproc.setMidiInputLocation(midiLocation.getChannel(),
								midiLocation.getCtrlType(), ((Integer) chan
										.getSelectedItem()).intValue(),
								((Integer) ctrl.getSelectedItem()).intValue());
					} else if(midiLocation instanceof MidiOutputLocation) {
						midiLocation.setMidiController(((Integer) chan
								.getSelectedItem()).intValue(),
								((Integer) ctrl.getSelectedItem()).intValue());
					}
				}
