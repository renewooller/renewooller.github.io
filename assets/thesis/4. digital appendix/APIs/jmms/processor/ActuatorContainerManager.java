 ...
 *  
 */
public class ActuatorContainerManager implements
     ...
     */
    public int getIndexOfSubContainer(
            ActuatorContainer sub) {
        int toRet = -1;
        for (int i = 0; i < acCount; i++) {
            if (aca[i] == sub) {
                toRet = i;
    ...
    public ActuatorContainer getActuatorContainer(
            ActuatorContainer[] a, String s) {
        for (int i = 0; i < a.length; i++) {
          
            if (a[i].toString().equalsIgnoreCase(s))
                return a[i];
        }
        // Err.e("the actuator container :" + s + "
        // doesn't exist in the array given");
        return null;
    }
    ...
    public Actuator getActuator(Actuator[] a, String s) {
        ...
        //  Err.e("the actuator :" + s + " doesn't exist
        // in the array given");
        return null;
    }
     ...
     *         otherwise
     */
    public boolean loadActuatorPath(ActuatorPath ap,
            String s) {

        ActuatorContainer[] ar = loadContainerPath(s);
        if (ap == null) {
            if (!unfinPaths.containsValue(ap))
                unfinPaths.put(s, ap);
            else
                PO.p("actuator path is null");
        ...
        if (ar == null) {
            if (!unfinPaths.containsValue(ap))
                ...
                PO.p("the list of actuator containers is null");
            return false;
        }
        ...
        if (ac == null) {
            if (!unfinPaths.containsValue(ap))
                ...
                PO.p("the list of actuators is null");
            return false;
        }
    ...
    public void loadUninishedPaths() {
        ...
        while (e.hasMoreElements()) {
            ...
            if (loadActuatorPath(
                (ActuatorPath) unfinPaths.get(key),
                (String) key) == false)
                Err.e("unable to load the unfinished path "
                        + (String) key);
        }
    }
     ...
     */
    public ActuatorContainer[] loadContainerPath(
            String s) {
       
        // trim it so that there is no crap in the beginning or end
        ...
        // set the first one to be the root
        ret[0] = this;
        //beacuse if it isn't the root we have a problem
        ...
        // go through and make the array
        for (int i = 1; i < ret.length; i++) {
            ret[i] = getActuatorContainer(
                ret[i - 1].getSubActuatorContainers(),
                sarr[i].substring(Save.st(
                    ActuatorContainer.sn)
                    .length()));
    ...
    public Actuator loadActuator(ActuatorContainer[] path, String a) {
        //trim the start and end tags off and find the
        // actuator
