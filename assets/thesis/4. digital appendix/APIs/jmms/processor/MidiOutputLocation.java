...
/*
 * this deals only with midi control messages, but could be upgraded to deal with other kinds such as pitch bend
 * @author wooller
 *
 *18/01/2005
 *
...
public class MidiOutputLocation implements Actuator, MidiLocation, Serializable {
	...
	private DefaultBoundedRangeModel chan = new DefaultBoundedRangeModel(1, 15, 1, 16);
	private DefaultBoundedRangeModel ctrlType = new DefaultBoundedRangeModel(0, 127, 0, 127);
	*/
	...
	public void commandRecieved(ActuatorEvent e) {
		//System.out.println("command recieved "+ e.toString());
		mom.getSequencer().sendControllerData(channel, ctrlType, 
				RMath.scaleValueInt(e.getMin(), e.getMax(), this.CTRL_MIN, this.CTRL_MAX, e.getValue()), 0);
	}
	...
	// an example of how it could be upgraded
