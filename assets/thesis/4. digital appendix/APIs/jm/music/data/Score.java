...
public class Score implements JMC, Cloneable, Serializable{
    ...
    */
	...
	//transient Object [] tparr;
	//transient int [] pcloc; // part channel locations
	//transient int pchcount = 0; // number of part with channel
	//transient Part [] toRetpc;
    ...
    public void fillWithChannelID(Vector tfill, int ich) {
        ...
        boolean [] chanDone = new boolean [this.size()];
        int [] nextNote = new int [this.size()];
        ...
        while(go) {
            ...
            for(int i=0; i<this.size(); i++) {
                // if the particular part is not exausted
                if(!chanDone[i]){
                    // if a current next note exists
                    if(nextNote[i] > -1) {
                        //PO.p("compare\n" + getPart(i).getPhrase(nextNote[i]).toString());
                      //  if(fore != null)
                      //  PO.p("to\n" + fore.toString());
                        // if the current note is more foremost
                        if((fore == -1 && getPart(i).getPhrase(nextNote[i]).getIdChannel() == ich) ||
                                getPart(i).getPhrase(nextNote[i]).getStartTime() <= 
                                    getPart(fore).getPhrase(nextNote[fore]).getStartTime()) {
                            fore = i; //set the foremost
                        }
                    } else {
                        nextNote[i] = 0; //initialise it
                    }
                    // find the next note in the channel/part
                    while(nextNote[i] < getPart(i).size()) {
                       PO.p("second while - score");
                        // if it isn't the one we want,
                        if(getPart(i).getPhrase(nextNote[i]).getIdChannel() != ich) {
                            nextNote[i]++; // try the next one
                        } else {
                            
                            PO.p("i = " + i+ "  | chid = " + ich);
                            
                            PO.p("next note = " + nextNote[i]);
                            // if it has the channel id we are looking for
                            // it may be a candidate for the foremost on the next loop
                            go = true;
                            //increment no more - we have the index we want
                            break;
                        }
                    }
                    // are we done?
                    if(nextNote[i]>=getPart(i).size()) {
                        chanDone[i] = true;
                        if(fore == i) // if this is the fore, make sure its 
                            fore = -1;
                    }
            ...
            if(fore != -1) {    
                // add the foremost phrase and move on to the next one
                tfill.add(getPart(fore).getPhrase(nextNote[fore]++).copy());
            }
    ...
    */
    ...
    public void fillWithChannelID(Vector tfill, int ich) {
        ...
        while(go) {
            go = false;
            int fore = -1;
            //search for the foremost
            for(int i=0; i< size(); i++) {
                Phrase chphr = getIdChPhrase(i, ich, pindex);
                if(chphr != null) {
                    //if this phrase is before the current foremost
                    if(fore == -1 ||
                            chphr.getStartTime() < getPart(fore).getPhrase(pindex[fore]).getStartTime()) {
                        fore = i;          
                    }
                }
            }
            if(fore > -1) {
                //add the fore and move the note to the next one
 ...
 //       PO.p("pc = " + pc + "  ic = " + ic);
   ...
   //         PO.p("nothing");
        ...
        while(pindex[pc] < getPart(pc).size() && pindex[pc] > -1) {
          //  
    //       PO.p("pindex[pc] = " + pindex[pc] + " size = " + getPart(pc).size());
  //         
        //   PO.p(getPart(pc).toString());
        //  PO.p(getPart(pc).getPhrase(pindex[pc]).toString());
            
            if(getPart(pc).getPhrase(pindex[pc]).getIdChannel() == ic &&
               !getPart(pc).getPhrase(pindex[pc]).getNote(0).isRest()) {
    ...
    * Specifies the Score's time signature
    ...
    //  private String nv = new String("aaa"); // a test variable - delete it
...
//this.getPartArray(), 0, this.getSize()-1);
    }
	...
	if(hi0 > lo0) {
	   /* Arbitrarily establishing partition element as the midpoint of
         ...
         while( lo <= hi ) {
             ...
             */
            while( ( lo < hi0 ) && 
		   ( a[lo].getChannel() < mid.getChannel() ) )
               ++lo;
             ...
             */
            while( ( hi > lo0 ) && 
		   ( a[hi].getChannel() > mid.getChannel() ) )
               --hi;
          ...
          */
         if( lo0 < hi )
            quickSortChan( a, lo0, hi );

         /* If the left index has not reached the right side of array
          * must now sort the right partition.
          */
         if( lo < hi0 )
            quickSortChan( a, lo, hi0 );
	}
    }
