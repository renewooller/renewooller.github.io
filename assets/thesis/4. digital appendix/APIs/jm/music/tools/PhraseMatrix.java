...
public final class PhraseMatrix implements JMC{
	...
	public Phrase generate(boolean p, boolean r, boolean d, int numOfNotes){
	    return generate(p, r, d, numOfNotes, null);
	}
	 ...
	 * @param d
	 * @param numOfNotes
	 * @param seedData an array of notes from which the new phrase will be generated.  The length
	 * 	of the array must be the same as the depth of the matrix.
	 * @return
	 */
	public Phrase generate(boolean p, boolean r, boolean d, int numOfNotes, Note [] seedData) {
	    if(seedData == null) {
	        seedData = notes;
	    } else if( seedData.length < this.dynamicDepth &&
	               seedData.length < this.pitchDepth &&
	               seedData.length < this.rhythmDepth) {
	        try{Exception e = new Exception("seed data must be >= any of the depths");
	        	e.fillInStackTrace();
	        	throw e;
		...
		for(int i=0;i<this.pitchDepth;i++){
			pitch[i] = seedData[i].getPitch();
		}

                // rhythm
        for(int i=0;i<this.rhythmDepth;i++){
            seedData[i].setRhythmValue(((int)(seedData[i].getRhythmValue()/0.25))*0.25);
        //    System.out.println("seedData rhythm " + seedData[i].getRhythmValue());
			for(int j=0;j<rhythmMap.length;j++){
				if(seedData[i].getRhythmValue() == rhythmMap[j]){
					rhythm[i] = j;
					break;
				}
			}
			...
			dynamic[i] = seedData[i].getDynamic();
		}
