...
public class Noise extends AudioObject{
	...
	*/
    ...
    private int gendynAmpGranularity = 100;//50; // more or less quantisation noise
	...
	public int work(float[] buffer)throws AOException{
		...
		case SMOOTH_NOISE:
                ...
                case GENDYN_NOISE:
                     for(;ret<buffer.length;){
                        ...
                        if(gendynIntArrayCounter >= gendynIntArrayLength) makeGendynArray();
                    }
                    break;
		default:
                    System.err.println(this.name+"jMusic error: Noise type " + noiseType + 
                            " not supported yet.");
                    System.exit(1);
		}
        ...
        private void makeGendynArray() {
            ...
            for(int i=0; i<gendynTime1; i++) {
                gendynIntArray[counter++] = (int)(gendynAmp0 + inc * i);
            }
            // stage 2
            inc = (gendynAmp2 - gendynAmp1) / gendynTime2;
            for(int i=0; i<gendynTime2; i++) {
                gendynIntArray[counter++] = (int)(gendynAmp1 + inc * i);
            }
            // stage 3
            inc = (gendynAmp3 - gendynAmp2) / gendynTime3;
            for(int i=0; i<gendynTime3; i++) {
                gendynIntArray[counter++] = (int)(gendynAmp2 + inc * i);
            }
        ...
        private int randWalk(final int prevVal, final int stepSize, final int mirror) {
            ...
            } else newVal = prevVal + (int)(Math.random() * stepSize * 2 - stepSize);
            // keep inside range
            while (newVal > mirror || newVal < mirror * -1) {
                if (newVal > mirror) newVal -= (newVal - mirror) * 2;
                if (newVal < mirror * -1) newVal += (mirror * -1 - newVal) * 2;
            }
            // avoid zero ????
	...
	*/
	public void setNoiseDensity(int newDensity) {
        ...
        */
        public void setGendynTimeMirror(double newVal) {
            if(newVal > 0) this.gendynTimeMirror = (int)newVal;
        }
        ...
        */
        public void setGendynAmpMirror(double newVal) {
            if(newVal > 0) this.gendynAmpMirror = (int)newVal;
        }
    ...
    public void setGendynTimeStepSize(int val) {
        if(val >= 0) this.gendynTimeStepSize = val;
    }
    ...
    public void setGendynAmpGranularity(int val) {
        if(val > 0) this.gendynAmpGranularity = val;
    }
