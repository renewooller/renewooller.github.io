...
public final class StereoPan extends AudioObject{
	...
	public int work(float[] buffer)throws AOException{
		...
		for(int i=0;i<returned;i++){
			if(channel==1){
                ...
                //if(this.pan > 0.5) buffer[i] = (buffer[i] *(float)(Math.min(1.0,(float)(Math.abs(Math.log(1.0 - this.pan) * 0.2))))); 
                 channel=2;
			}else{
                if(this.pan < 0.5) buffer[i] = (buffer[i] * (float)(this.pan * 2.0));
                //if(this.pan < 0.5) buffer[i] = (buffer[i] *(float)(Math.min(1.0,(float)(Math.abs(Math.log(this.pan) * 0.2))))); 
                channel=1;
			}
		}
		return returned;
     ...
     */
        public void setPan(double pan){
            if(pan >= 0.0 && pan <= 1.0) this.pan=(float)pan;
        }
}
